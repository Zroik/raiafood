                ...
                <div className="bg-white p-6 rounded-2xl border border-gray-100 shadow-sm">
                    <form onSubmit={handleFilter} className="flex flex-col sm:flex-row items-stretch gap-3">
                        <input
                            ...
                            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-200 text-sm focus:border-violet-400 focus:ring-2 focus:ring-violet-100 outline-none"
                        />
                        <select
                            value={status}
                        ...
                        <button type="submit" className="px-6 py-2.5 bg-violet-600 hover:bg-violet-700 text-white rounded-xl text-sm font-semibold transition-all shadow-md shadow-violet-100">
                            Filter
                        </button>
                    </form>
                </div>
                        ...
                        <table className="w-full text-left border-collapse">
                            <thead>
                                <tr className="bg-gray-50 text-[10px] uppercase font-bold text-gray-400 tracking-wider">
                                    ...
                                    <th className="py-4 px-6">Status Kirim</th>
                                ...
                                {orders.data.length === 0 ? (
                                    <tr>
                                        <td colSpan="7" className="py-12 text-center text-gray-400">
                                            Tidak ada data transaksi pesanan ditemukan.
                                        </td>
                                    </tr>
                                ) : (
                                    orders.data.map(order => (
                                        <tr key={order.id} className="hover:bg-gray-50/50 transition-colors">
                                            ...
                                            <td className="py-4 px-6">
                                                <div>
                                                    <p className="font-semibold text-gray-800">{order.user ? order.user.name : order.shipping_address?.name || 'Tamu'}</p>
                                                    <p className="text-xs text-gray-400">{new Date(order.created_at).toLocaleDateString('id-ID', { day: 'numeric', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' })}</p>
                                                </div>
                                            </td>
                                            <td className="py-4 px-6 font-bold text-gray-900">Rp {Number(order.total).toLocaleString('id-ID')}</td>
                                            <td className="py-4 px-6">
                                            ...
                                            <td className="py-4 px-6 text-gray-600 uppercase text-xs font-semibold">{order.payment_method}</td>
                                            <td className="py-4 px-6">

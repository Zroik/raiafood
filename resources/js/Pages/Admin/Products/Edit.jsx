...
export default function Edit({ product, categories }) {
    const { data, setData, post, processing, errors } = useForm({
        _method: 'PUT', // Laravel method spoofing for file uploads on update
        name: product.name || '',
        category_id: product.category_id || '',
        description: product.description || '',
        short_description: product.short_description || '',
        price: product.price ? Math.round(Number(product.price)) : '',
        discount_price: product.discount_price ? Math.round(Number(product.discount_price)) : '',
        stock: product.stock || '',
    ...
    return (
        <AdminLayout header={<h2 className="text-xl font-semibold leading-tight text-gray-800">Edit Produk Cookies</h2>}>
            <Head title="Edit Produk" />

            <div className="max-w-3xl mx-auto bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                <div className="p-6 border-b border-gray-50 flex items-center justify-between">
                    <h3 className="text-md font-bold text-gray-800">Edit Detail: {product.name}</h3>
